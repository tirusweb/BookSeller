...
import { apiGetBillByStatus } from "../../services/Bill/Bill";
...
const MyOder = () => {
  ...
  const handleByStatus = async () => {
    try {
      const response = await apiGetBillByStatus(idcus, status);
      console.log(response.data);
      if (response.data.status === 1) {
        setMyoder(response.data.books);
      } else {
        setError("Lỗi");
      }
    } catch (error) {
          ...
          <div className=" grid grid-cols-10 gap-2">
            <div className="col-span-10 bg-white rounded shadow-lg">
              <div className=" flex items-center justify-between">
                ...
                <select
                  className="text-sm font-medium mr-12 border border-solid mt-2  border-gray-300   px-6 py-2 rounded"
                  value={status}
                  onChange={(e) => {
                ...
                >
                  ...
                  <option value="Đã đặt">Đã hủy</option>
                  <option value="Đã giao">Đã đặt</option>
                  <option value="Đã hủy">Đã giao</option>
                </select>
              </div>
